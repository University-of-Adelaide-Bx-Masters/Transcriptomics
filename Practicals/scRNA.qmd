...
This pipeline has already aligned reads to the genome using STAR and counted individual UMIs for each gene.
...
In addition to the above all barcodes returning a UMI count of zero have been removed, ensuring the data is a little smaller and more workable than the original (>300,000 barcodes).
...
Let's create a new R Project called `scRNA` inside your transcriptomics folder.
Making sure you are in the correct project folder, create a new directory called `data`
...
Once you've formed this folder and project, head to the `Terminal` and using bash, download today's data from [this link](data/scrna_practical_data.tar.gz) using `wget`.

Once you have obtained this file, please extract it using the following command.

```bash
cd data
tar -xzvf scrna_practical_data.tar.gz
```
...
```bash
ls -alh data/raw_feature_bc_matrix
```

```{bash, echo = FALSE}
ls -alh data/raw_feature_bc_matrix | sed -r 's/(stevie)(.+)(stevie)/my_userid\2myuser_id/g'
```
...
These are checksums (or mds5sums) which will be different to the values shown below if any content within a files has changed.

```{bash}
md5sum data/raw_feature_bc_matrix/*
```
...
Take a moment to look at all the information printed to the screen

::: {.callout-important}
*In this initial object, how many genes do we have and many cells do we have?*
:::
...
To really check our results, we can use and FDR filter to provide support for a droplet being considered as containing a cell.

```{r}
subset(emptyBC, FDR < 0.01)
```

As you can see, this has identified `r sum(emptyBC$FDR < 0.01, na.rm = TRUE)` droplets as containing a cell.
Notice that the original (unfiltered) data was `r comma(nrow(emptyBC))` barcodes, so we have a fairly high rate (`r percent(1 - sum(emptyBC$FDR < 0.01, na.rm = TRUE) / nrow(emptyBC), 0.1)`) of empty droplets.
...
For this step, and for downstream analysis, we'll need to know which gene is which.
...
Unfortunately, 10X genomics have not made this information public so we can't be sure, but testing seemed to check this guess out.

Head to https://ftp.ensembl.org/pub/release-93/gtf/homo_sapiens/ and download the file `r list.files("data", pattern = "gtf.gz")` into the directory 'data'.
(Use `wget` once you have the correct download path)

Once this has downloaded, load the gene-level information.

```{r}
genesGR <- import.gff("data/Homo_sapiens.GRCh38.93.gtf.gz", feature.type = "gene")
...
We can see that we have UMI counts for 13 mitochondrial genes and we can use these to assess whether a cell contains an excessive amount of mitochondrial RNA, as an indicator of a cell which has ruptured.
...
We can add this directly to the colData in our `SingleCellExperiment` object, using the arbitrarily chosen value of 200.
...
Now we have some cells which appear to be outliers, with some being nearly 100% mitochondrial.
...
By this strategy, we would lose `r sum(sceWC$hiMito)` cells.
Let's see how this relates to the small library sizes

```{r}
colData(sceWC) %>%
  as.data.frame() %>%
  dplyr::count(lowCounts, hiMito)
```

Combining these two filtering approaches, we would now retain `r sum(!sceWC$lowCounts & !sceWC$hiMito)` cells for downstream analysis.
...
sceWC$discard <- colData(sceWC) %>%
  with(
    lowCounts | hiMito | scDblFinder.class == "coublet"
  )
colData(sceWC) %>%
  as.data.frame() %>%
  dplyr::count(
    lowCounts, hiMito, scDblFinder.class, discard
  )
```

Checking all of our QC criteria we would now have `r sum(!sceWC$discard)` cells to keep for clustering.
...
We can take a couple of different approaches to normalisation, and today we'll use the deconvolution approach.
...
We should choose the elbow point on this plot and although there are automated approaches, I'd be comfortable choosing 5 by looking at that plot.
Let's remove the un-informative PCs.

```{r}
reducedDim(sceFilt, "PCA") <- reducedDim(sceFilt, "PCA")[,1:5]
```
